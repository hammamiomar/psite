...
Everything here was built with Claude as a pair programmer. More on what that collaboration actually looked like at the end.
...
No loops. No scheduler state machine. No dynamic tensor shapes. This is why the whole thing can live inside a single CUDA graph.
...
**Hooks break `torch.compile(fullgraph=True)`.** They're Python callbacks — opaque to Dynamo's tracer. Each hook creates a graph break. With CUDA graphs, the GPU would need to stop, signal the CPU, run Python, then resume. Instant death for 50 FPS.
...
{/* Screenshot: the four blocks in the UI, each linked to a different stem, each with a different feature active */}
...
We had features. We didn't know what they meant.
...
Complete failure. Max confidence across all 20,480 features: 0.19. Known feature 2301 ("intense/evil") got labeled "crowd" at 0.11. Root cause: single seed + empty prompt means every image starts from the same grey blob. Features need semantic context to express themselves — the CLIP direction captures "how this grey blob changed" rather than what the feature actually means.

#### What Worked: A VLM Ensemble

The insight was that no single method works for all four blocks. Composition features (down.2.1) are easy for VLMs to describe. Spatial structure features (mid.0) need spatial analysis, not visual description. Texture features (up.0.1) are the hardest — even three VLMs struggle to verbalize visual patterns.
...
{/* Screenshot: the FeaturePicker browsing features with labels, categories, and confidence indicators */}

**Three VLMs via OpenRouter** with block-specific prompts. This matters enormously — a generic "describe what's different" prompt fails, but "IGNORE the subject matter, focus ONLY on visual properties" for the texture block succeeds. Qwen3-VL-235B first ($12), GLM-4.6V second ($27), Kimi K2.5 as tiebreaker ($36).
...
The defining architectural decision: _all DSP runs at upload time._ Runtime is O(1) array interpolation.
...
**HPSS** — harmonic-percussive source separation via median filtering in the spectrogram domain. The ratio gates subsequent analysis: if a stem is >70% percussive, we skip harmonic features entirely (no point tracking pitch on a drum stem).
...
**Pitch tracking** — PESTO (monophonic, 12x faster than CREPE) for vocals and bass. Basic Pitch (Spotify, polyphonic ONNX) for the "other" stem. Output: pitch_hz, pitch_confidence, pitch_normalized [0,1] for spatial mapping.
...
**Phase Locking Value (PLV)** — Hilbert transform to extract instantaneous phase, then compute phase synchrony in a sliding window. 1.0 = perfectly locked.
...
Everything in Part I exists to produce two things: (1) a compiled CUDA graph that can steer SDXL-Turbo's generation via SAE features at four attention blocks, and (2) pre-computed audio feature arrays for every stem. Part II is about the _instrument_ — how those features become an interactive tool for navigating the model's space through music.
...
{/* Screenshot: a Block Config Panel fully expanded, with feature picker, spatial grid, and all controls visible */}
...
{/* Screenshot: FeaturePicker searching for "warm" — showing results like "warm amber lighting," "warm wood grain," "warm golden hour" */}
...
{/* Screenshot: the SpatialGrid canvas with a custom hand-drawn mask — maybe a diagonal sweep */}
...
{/* Screenshot: split view showing pitch-aligned mode — low note affecting bottom, high note affecting top */}
...
Directly mapping audio amplitude to visual intensity is mechanical and jittery. Raw signal → raw parameter = a bad visualizer. The bridge sits between audio features and SAE steering, transforming reactive signals into _organic motion_.
...
{/* Screenshot: the Destination Panel showing two prompts, blend slider, and reactive mode active */}
...
{/* Screenshot: the four orbs in the belly, with one glowing brighter than the others — showing prominence in action */}
...
{/* Screenshot: the full belly scene — CrystalHeart in the center, four flower orbs around it, tendrils connecting them */}
...
- **SAE encoder weights waste ~104MB VRAM.** We only use the decoder at inference. The encoder loads then gets deleted — but peak allocation still happens during loading.
- **HPSS peaks at ~1.4GB GPU per stem** during offline processing. Four stems = 5.6GB transient.
- **The `linear` physics preset** (k=2500, m=1.0) sits at the numerical stability boundary for semi-implicit Euler at 50 FPS. It works, but it's the least robust preset.
- **77.4% label confidence** means ~22% of features have medium or low-confidence labels. The texture block (up.0.1) is weakest at 62% high-confidence.
- **Single-seed composition** — the circular walk explores a 2D subspace of noise space. Adding more seeds (3D+ walk) would increase diversity but complicate the beat-sync math.
- **512x512 output** — SDXL-Turbo at 512x512 is the sweet spot for speed. Higher resolution would require either a different model or upscaling.

---

### What Claude Looked Like as a Pair Programmer

This matters for understanding what ML engineering looks like in 2026.
...
The point: this is _pair programming_, not code generation. I provided direction and taste — what should this feel like, what's the right abstraction, where to simplify. Claude provided implementation velocity — writing cleanly, catching edge cases, parallelizing work. The result is a codebase neither of us would have built alone.
