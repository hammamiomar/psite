...
That led me to Anthropic's work on [Sparse Autoencoders](https://www.anthropic.com/research/mapping-mind-language-model). SAEs decompose model activations into interpretable feature dictionaries. Models store concepts in superposition— distributed across many neurons. SAEs untangle that into individual features you can actually understand.

And you can *steer* generation with these features.

The researchers at EPFL showed this with [SDXL-unbox](https://sdxl-unbox.epfl.ch/). SAE features from different UNet blocks control different semantic aspects of image generation. Feature 2301 in the down blocks makes images more "intense." Feature 4977 in the up blocks adds "tiger stripes." Actual concepts you can manipulate.

So I thought: what if I could connect this to audio?

I trained my own SAE on drum audio representations: [drums_SAE](/blog/drums-sae). It worked. I could extract interpretable features from audio embeddings. That was a good sign.
